...
interface GeneratedArticleWithRoute {
...
async function writeArticle(blog: Blog, plan: ArticlePlan, items: SourceCandidate[]): Promise<GeneratedArticleWithRoute> {
  ...
  const generated = await aiJsonWithMeta<GeneratedArticle>(
  ...
  return { article: generated.value, ai: generated.meta };
}
...
async function refreshExistingPost(blog: Blog, candidate: RefreshCandidate, started: string): Promise<{ blog: string; status: string; title?: string } | null> {
  ...
  const generated = await aiJsonWithMeta<RefreshHeadlinePlan>(
  ...
  if (generated.meta.route === "fallback" && policy === "review") {
    recordRun(
      blog.id,
      "content-refresh",
      "ok",
      "Skipped automatic published-headline mutation because the proposal was generated by fallback AI",
      { publicationId: candidate.publication.id, aiRoute: generated.meta, fallbackContentPolicy: policy },
      started,
    );
    return null;
  }
...
async function runOne(blog: Blog, force = false): Promise<{ blog: string; status: string; title?: string }> {
  ...
  try {
    ...
    const generated = await writeArticle(blog, plan, items);
    ...
    const policy = fallbackContentPolicy();
    const fallbackForcedReview = generated.ai.route === "fallback" && blog.publishMode === "auto" && policy === "review";
    const publishBlog: Blog = fallbackForcedReview ? { ...blog, publishMode: "review" } : blog;
    const credentials = decryptJson<unknown>(blog.credentialsCipher);
    const result = await platformAdapter(blog.platform).publish(publishBlog, credentials, article);
    ...
    setLastRun(blog.id);
    const message = fallbackForcedReview
      ? `Fallback-generated article was forced to review draft: ${article.title}`
      : `Published ${article.title}`;
    recordRun(blog.id, "editorial", "ok", message, {
      plan,
      result,
      force,
      experiment: plan.experiment,
      aiRoute: generated.ai,
      fallbackContentPolicy: policy,
      fallbackForcedReview,
    }, started);
