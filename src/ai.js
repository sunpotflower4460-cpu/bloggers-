...
export class OpenAICompatibleProvider {
  ...
  async decide({ blog, posts, metrics }) {
    const content = await this.#complete([
      {
        role: 'system',
        content: 'You are the editorial director of one blog. Return JSON only. Never optimize for article count; WAIT is valid.',
      },
      {
        role: 'user',
  ...
  async draft({ blog, decision }) {
    const content = await this.#complete([
      {
        role: 'system',
        content: 'Write a useful blog draft. Follow the supplied editorial policy and voice. Do not fabricate facts or sources.',
      },
      {
        role: 'user',
        content: JSON.stringify({
